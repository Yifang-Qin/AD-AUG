...
def remap_user_item(data, mp_item):
    dd = data.copy()
...
if __name__ == '__main__':
    ...
    train_data.to_csv(root_path + 'ml-1m_1/train.csv', index=False)
    valid_data.to_csv(root_path + 'ml-1m_1/valid.csv', index=False)
    test_data.to_csv(root_path + 'ml-1m_1/test.csv', index=False)

    unique_uid = pd.unique(all_data['uid'])
    with open(root_path + 'ml-1m_1/unique_uid.txt', 'w') as f:
        for uid in unique_uid:
            f.write('%s\n' % uid)

    unique_sid = pd.unique(all_data['sid'])
    with open(root_path + 'ml-1m_1/unique_sid.txt', 'w') as f:
