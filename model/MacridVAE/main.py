...
def valid_vae(vad_data_tr, vad_data_te, VAE, arg, device, save_emb):
    VAE.eval()
    ...
    for bnum, st_idx in enumerate(range(0, n_vad, arg.batch)):
        ...
        if save_emb:
            z_ = VAE.encode(x, is_train=False)
            z_s.append(z_)

        logits, _, _, _ = VAE(x, is_train=False)
    ...
    if save_emb:
        z = torch.cat(z_s, dim=1) #.view(-1, arg.dfac)
        np.save('z_ori.npy', z.detach().cpu().numpy())
...
Best Recall@50:        {}
    ...
    VAE = MacridVAE(arg, n_items, device).to(device)
    ...
    for epoch in range(arg.epoch):
        ...
        for bnum, st_idx in enumerate(range(0, n_train, ARG.batch)):
            ...
            regs.append(reg_var.detach().cpu().numpy())
        ...
        ndcg100, recall20, recall50 = valid_vae(train_data, valid_data, VAE, arg, device, False)
        ...
        if ndcg100 > best_ndcg100:
            ...
            if arg.save_name is not None:
                ...
                torch.save(VAE.state_dict(), arg.save_name)
            ndcg_te100, recall_te20, recall_te50 = valid_vae(train_data, test_data, VAE, arg, device, False)
...
if __name__ == '__main__':
    ...
    if ARG.mode == 'trn':
        ...
        if sparse.isspmatrix(train_data):
            train_data = train_data.toarray()
        train_data = torch.Tensor(train_data.astype(np.float32)).to(device)
        model = MacridVAE(ARG, n_items, device).to(device)
