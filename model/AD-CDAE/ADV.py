...
class ADV(nn.Module):
    ...
    def forward(self, u, x, anneal, opt, opt_aug, is_train=False):
        ...
        # aug_graph[x == 0] = (1-gumbel)[x == 0]
        # aug_graph[x == 1] = gumbel[x == 1]
        # aug_graph = x * gumbel
        # aug_reg = torch.zeros_like(x).to(self.device)
        # aug_reg[x == 0] = (1 - (1 - x) * aug)[x == 0]
        # aug_reg[x == 1] = (x * aug)[x == 1]

        z_aug = self.D.encode(u, aug_graph, is_train=is_train)
        z_ori = self.D.encode(u, x, is_train=is_train)
        mi_aug = self.calc_I(z_ori, z_aug)

        add = torch.norm(aug_graph[x == 0], p=1) / (x == 0).sum()
        drop = torch.norm(1 - aug_graph[x == 1], p=1) / (x == 1).sum()
        # add = torch.norm(aug[x == 0], p=1) / (x == 0).sum()
        # drop = torch.norm((1 - aug)[x == 1], p=1) / (x == 1).sum()

        # add = aug_graph[x == 0].sum() / (x == 0).sum()
        # drop = (1 - aug_graph[x == 1]).sum() / (x == 1).sum()
        reg_aug = add + drop
        # reg_aug = drop
        # reg_aug = (x - aug_graph).sum() / (x == 1).sum()
        # drop = reg_aug
        # add = reg_aug

        aug_loss = self.arg.alpha_aug * mi_aug + self.arg.rg_aug * reg_aug
        # aug_loss = self.arg.rg_aug * reg_aug
        ...
        _, recon, z_ori = self.D(u, x, x, is_train=is_train)
