...
ARG.add_argument('--decay', type=float, default=0.98,
                 help='learning rate decay rate')
ARG.add_argument('--decay_step', type=int, default=50,
...
Best Recall@50:        {}
    ...
    scheduler = lr_scheduler.StepLR(opt, step_size=arg.decay_step, gamma=arg.decay)
    scheduler_aug = lr_scheduler.StepLR(opt_aug, step_size=arg.decay_step, gamma=arg.decay)

    update_count = 0
    num_batches = int(np.ceil(float(n_train) / arg.batch))
    total_anneal_steps = 200 * num_batches
    ...
    for epoch in range(arg.epoch):
        ...
        for bnum, st_idx in enumerate(range(0, n_train, arg.batch)):
            ...
            x = train_data[user_list].to(device)
...
if __name__ == '__main__':
    ...
    print(f'\tn_users: {n_users}\n\tn_items: {n_items}\n\t'
          f'Train size: {train_data.shape[0]}\n\t'
          f'Valid size: {valid_data.shape[0]}'
