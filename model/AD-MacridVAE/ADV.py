...
class ADV(nn.Module):
    ...
    def forward(self, x, anneal, opt, opt_aug, is_train=False):
        ...
        # _, recon, kl, z_aug = self.D(aug_graph, aug_graph, is_train=is_train)
        ...
        # aug_loss = self.arg.alpha_aug * mi_aug + 0.1 * recon + anneal * kl
    ...
    #     sim_matrix = torch.exp(sim_matrix / temperature)
    #
    #     sim_matrix1 = torch.einsum('mil,njl->mnij', x1, x1) / torch.einsum('mi,nj->mnij', x1_abs, x1_abs)
    #     sim_matrix1 = torch.exp(sim_matrix1 / temperature)
    #
    #     sim_matrix2 = torch.einsum('mil,njl->mnij', x2, x2) / torch.einsum('mi,nj->mnij', x2_abs, x2_abs)
    ...
    #     pos_sim1 = pos_sim1[range(K), range(K), :]
    #
    #     pos_sim2 = sim_matrix2[:, :, range(batch_size), range(batch_size)]
    #     pos_sim2 = pos_sim2[range(K), range(K), :]
    #
    #     # Shape: K * batch_size
    ...
    #
    #     loss_0 = pos_sim / (sim_matrix1.sum(dim=0).sum(dim=-1) - pos_sim1)
    #     loss_1 = pos_sim / (sim_matrix2.sum(dim=0).sum(dim=-1) - pos_sim2)
